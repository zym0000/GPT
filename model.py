...
class GPTModule(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        print(f"loading weights from pretrained type: {model_type}")
        ...
        sd_keys_hf = [k for k in sd_hf.keys() 
                    ...
                    #and not k.endswith('attn.c_attn.bias')
                    #and not k.endswith('attn.c_proj.bias')]  
        
        transposed = ['attn.c_attn.weight', 'attn.c_proj.weight',
                    'mlp.c_fc.weight', 'mlp.c_proj.weight']

        for k in sd_keys_hf:
            is_transposed = any(k.endswith(w) for w in transposed)
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt, world_size=None):
        ...
        # 2. 计算每次前向+反向的 FLOPs
        # 参考 PaLM 论文 Appendix B: https://arxiv.org/abs/2204.0231
